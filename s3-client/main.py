...
def put_object(config: AWSConfig, uri_path: str, body: bytes) -> Response:
    return send_aws_http_request(config, "PUT", uri_path, {}, {}, body)


def get_object(config: AWSConfig, uri_path: str) -> Response:
    return send_aws_http_request(config, "GET", uri_path, {}, {})
    ...
    def handle_request(self, request: Request) -> Response:
        ...
        try:
            if request.method == "GET":
                response = get_object(config, uri_path)
            elif request.method == "PUT":
                response = put_object(config, uri_path, payload)
            elif request.method not in ["GET", "PUT"]:
                return Response(
                    200,
                    {"content-type": "text/plain"},
                    bytes(f"Request method {request.method} not supported. Please use 'PUT' or 'GET'.")
                )
